...
layout = html.Div([content_first_row,
                   html.Div(id='option-chart-output-id',
                            children=[dcc.Loading(dcc.Graph(id='option-chart-output', figure={}), type='default')],
                            style={'display': 'none'}),
...
def display_click_data(target_closing_price, clickData, n_intervals, n_clicks, switch_value, replay_history, figure,
                       target_close,
                       start_date, end_date):
    tickr.target_close_lst.append(target_close)
    target_close_text = 'Target Closing Price (Modeled) : "{}"'.format(
        ', '.join([str(i) for i in list(set(tickr.target_close_lst)) if i]))
    ...
    try:
        ...
        if ctx.triggered[0]['prop_id'] == 'graph.clickData':  # just clicking chart
            ...
            return dash.no_update, oc.generate_fig(), target_close_text, toggle_display(
            ...
            return tickr.get_charts(), dash.no_update, target_close_text, toggle_display(
                'showOptionHistory'), toggle_display('ReplayHistory'), dash.no_update
        elif ctx.triggered[0]['prop_id'] == 'target_close.value':  # triggered by changing target_close
            tickr.target_close = target_close
            tickr.get_lastSalePrice()
            tickr.predict()
            return tickr.fig, dash.no_update, target_close_text, toggle_display('showOptionHistory'), toggle_display(
                'ReplayHistory'), dash.no_update
            ...
            return dash.no_update, dash.no_update, dash.no_update, toggle_display('showOptionHistory'), toggle_display(
                'ReplayHistory'), dash.no_update
